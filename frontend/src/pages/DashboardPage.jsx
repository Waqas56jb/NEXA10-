...
export default function DashboardPage() {
  ...
  const [earning, setEarning] = useState(() => localStorage.getItem('nexa10_earning') === '1');
  ...
  useEffect(() => {
    if (!earning) return;
    const id = setInterval(() => setBalance((b) => b + 0.00000001), 100);
    return () => clearInterval(id);
  }, [earning]);
  ...
  const toggleEarn = () => {
    const next = !earning;
    setEarning(next);
    localStorage.setItem('nexa10_earning', next ? '1' : '0');
  };
    ...
    <>
      ...
      <main className="main">
        ...
        <section className="card ref-card fade-in d3">
          <div className="ref-info"><div className="ref-title">Your Referral Link</div><div className="ref-url">{REF_LINK}</div></div>
          <div className="ref-actions">
            <button className="ref-btn copy" type="button" onClick={copyRef}>Copy</button>
          </div>
        </section>
        <section className="summary-row fade-in d3">
          <div className="card summary-tile"><span className="summary-label">Deposit</span><span className="summary-amount">$0.40</span></div>
          <div className="card summary-tile"><span className="summary-label">Earnings</span><span className="summary-amount green">$0.00</span></div>
        </section>
